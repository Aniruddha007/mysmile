...
//$userList = $this->admin_common_model->get_all('category_service');
$userList = $this->admin_common_model->get_where('provider_services',['remove_status'=>'No']);
            ...
            <div class="content-page">
                ...
                <div class="content">
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <h4 class="pull-left page-title" >Welcome <?php $fetch_app_name = $this->admin_common_model->get_where('admin',['id'=>'1']);
                                echo $fetch_app_name[0]['app_name']; ?> Admin</h4>&nbsp;	&nbsp;	
...
<div class="row">
    ...
    <form action="<?php echo base_url(); ?>admin/goProService" method="POST" id="bulk_form" onsubmit="delete_confirm()">
...
-->                                                            <th>Service Price</th>
                                              ...
                                              $busi = $this->admin_common_model->get_where('category',['id'=>$row['category_id']]);
                                              $busi1 = $this->admin_common_model->get_where('sub_category',['id'=>$row['sub_cat_id']]);
                                              $busi2 = $this->admin_common_model->get_where('users',['id'=>$row['user_id']]);
                            ...
                            <td align="center"><input type="checkbox" name="checked_id[]" class="checkbox" value="<?php echo $row['id']; ?>"/></td>  
                                                            ...
                                                            <td><?= $busi2[0]['first_name'].' '.$busi2[0]['last_name']; ?></td>
                                                            <td><?= $row['service_name']; ?></td>
<!--                                                            <td><?= $row['service_name_ar']; ?></td>
...
-->                                                           
                              ...
                              <!-- <td><select name="status" getId1="<?=$row['id'];?>"   style="height: auto !important;">
											...
											<option <?php if($row['status']=='Deactive'){ echo 'selected'; } ?>>Deactive</option> 
											</select>
										</td>-->
										 
                                                           <!-- <td class="text-center"><a href="<?=base_url('admin/view_page/add_category_service/'.$row['id']);?>"><button class="btn view-t"></a> </button>   </button> <button class="btn delete-t"><i class="fa   fa-trash-o" onclick="deleteUsers('<?=$row["id"];?>')"></i></button></td>
                                                          --> 
<!--                                                            <td class="text-center"><a href="<?=base_url('admin/view_page/add_category_service/'.$row['id']);?>"><button class="btn view-t"><i class="fa  fa-edit"></i></a> </button>   </button> <button class="btn delete-t"><i class="fa   fa-trash-o" onclick="deleteUsers('<?=$row["id"];?>')"></i></button></td>
-->
                                                        </tr>
<?php $i++; } ?>
...
function(){
        ...
        $.ajax({
          url: "<?=base_url('admin/remove_status');?>",
          data: {'table': 'provider_services', 'id': id}, // change this to send js object
          type: "POST",
