...
$driverList = $this->admin_common_model->get_where('users',[type=>'DRIVER',driver_type=>'CABS']);
...
?>
      ...
      <a href="<?=base_url('admin/view_page/ExUser?type=cabs_drivers');?>"><button type="button" class="btn btn-purple waves-effect waves-light" > Export csv</button></a>
...
$car = $this->admin_common_model->get_where('car_list',['id'=>$row['car_id']]);?>
  ...
  <td><select name="available_status" getId="<?=$row[id];?>"   style="height: auto !important;">
										  ...
										  <td><select name="status" getId1="<?=$row[id];?>"   style="height: auto !important;">
											<option  <?php if($row['status']=='Active'){ echo 'selected'; } ?>>Approve</option>
											<option <?php if($row['status']=='Deactive'){ echo 'selected'; } ?>>Disapprove</option> 
											</select>
										</td>
										
										
										<td><?= $row['date_time']; ?></td>
 <td class="text-center"><a href="<?=base_url('admin/view_page/add_driver1/'.$row['id']);?>"><button class="btn view-t"><i class="fa  fa-edit"></i></a> </button>  <a href="<?=base_url('admin/view_page/view_driver/'.$row['id']);?>"><button class="btn view-t"><i class="fa  fa-eye"></i></a> </button> <button class="btn delete-t"><i class="fa   fa-trash-o" onclick="deleteUsers('<?=$row["id"];?>')"></i></button></td>
